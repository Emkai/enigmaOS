...
Item {
    ...
    // keyboard.capsLock is unreliable under Wayland (verified: it reads
    // false even when the real LED is on, and never changes afterward), so
    // it can't be used at all, live or as a one-time read. The only
    // consistently correct signal is the Key_CapsLock keypress itself.
    // It's caught per-component (see LoginScreen, LockScreen, PowerMenu,
    // SessionSelector, UserSelector, LayoutSelector - each is a separate
    // focus target, so each needs its own handler) and all of them funnel
    // through this single toggle so there's exactly one place to reason
    // about the state.
