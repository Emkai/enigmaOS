    ...
    Item {
        ...
        Item {
            ...
            Text {
                ...
                visible: text !== "" && loginScreen.state !== "authenticating"
                opacity: visible ? 1.0 : 0.0
            ...
            // Dedicated, always-on caps lock indicator. Bound directly to the
            // live keyboard state (root.capsLockOn) rather than routed through
            // loginMessage.warn(), so it can't be missed, overwritten by a
            // login-failed message, or left stale if a toggle event is missed.
            Text {
                id: capsLockMessage
                text: textConstants.capslockWarning || "Caps Lock is active"
                font.pixelSize: Config.warningMessageFontSize * Config.generalScale
                font.family: Config.warningMessageFontFamily
                font.weight: Font.Bold
                color: Config.warningMessageWarningColor
                visible: root.capsLockOn && loginScreen.userNeedsPassword && loginScreen.state !== "authenticating"
                opacity: visible ? 1.0 : 0.0
                anchors.top: loginMessage.visible ? loginMessage.bottom : loginArea.bottom
                anchors.topMargin: visible ? Config.warningMessageMarginTop : 0
